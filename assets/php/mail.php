...
if (isset($_POST['action']) && $_POST['action'] == 'afspraakInfo') {
    ...
    if (!empty($naam) || !empty($email) || !empty($telefoon) || !empty($datum) || !empty($tijd) || !empty($aantal_personen) || !empty($soort_service) || !empty($service) || !empty($prijs)) {
        ...
        $mail->addAddress('tgeknipt@gmail.com');
   ...
   <p>Soort service : <b>' . $soort_service . '</b></p>
    ...
    &afspraak_service=' . $service . '&naam=' . $naam . '&opmerking=' . $opmerking . '&aantal=' . $aantal_personen . '&tel=' . $telefoon . '&loc=' . $soort_service . '&prijs='.$prijs.'">Klik hier om de afspraak te bevestigen</a>
    ';
        $message = '';
        if (!$mail->send()) {
            $message .= 'fail';
